...
curl -s $BASE_URL/test | head -20
echo ""

echo "2. Store products endpoint"
curl -s $BASE_URL/store/products?limit=5 | head -20
echo ""

echo "3. Store masters endpoint"  
curl -s $BASE_URL/store/masters?limit=3 | head -20
echo ""
...
curl -X POST $BASE_URL/store/product-requests \
  ...
  -d '{
    ...
    "quantity": 2,
    "notes": "Красный лак для тестирования"
  }' | head -20
